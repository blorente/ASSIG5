...
public class ComplicaMove extends Move {
	...
	public void executeMove(Board board) throws InvalidMove {
		...
		if (!Util.isColumnValid(board, this.getCol())) {
			...
			if (h < Util.ERRORTHRESHOLD) { // if the column is full I move all
											// the Counters one cell down and do
											// the first cell empty
			...
			board.setPosition(this.getCol(),
					...
					this.getPlayer());// to put the counter in the first empty
										// position			
		}
	}
	...
	public void undo(Board board) {
		if (this.out != Counter.EMPTY) {
			board = this.moveUpCells(board, this.getCol()); // if the column is
															// full I move the
															// counters one up
															// to put the first
															// counter out of
			...
			board.setPosition(this.getCol(), (h + 1), Counter.EMPTY); // the
																		// same
																		// undo
																		// as C4
		}
